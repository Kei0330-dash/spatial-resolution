...
outputFrame::outputFrame(const TGWindow *p, const TGWindow *mainWindow, UInt_t w, UInt_t h, param parameter)
	...
	ADCsumLabel = new TGLabel(output_Frame, Form("ClusterXXX of ADC Sum:  Not Found"));
	...
	output_Frame->AddFrame(ADCsumLabel, new TGLayoutHints(kLHintsCenterX | kLHintsCenterY, 10, 10, 10, 10));
...
void outputFrame::EntryChanged(Long_t val) {
	...
	if(entry_num < max_size){
		...
		ADCsumLabel->SetText(Form("Cluster%d of ADC Sum:  %d", entry_num, params.out.cluster[entry_num].Get_ADCsum()));
		...
		ADCsumLabel->SetText(Form("Cluster%d of ADC Sum:  Not Found", entry_num));
